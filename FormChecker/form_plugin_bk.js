...
<!-- check if all email have a correct syntax   --> 
 if (continue_submit) {  
    ...
    settings.InvalidEmailFunction($(this)); continue_submit=false; return false
  }
    }
     }
  }
...
/*is equal */
}else {  settings.NotEqualFunction($(this));
  continue_submit=false; return false ;}
...
//only number key press ------------------------- START ----------------------------------  
     ...
     if (max_length) {
      ...
      if((longeur<max_length)|(Selected.length>0)){
     }else {
      return false;
     }
     };
    ...
    if ($(this).attr(settings.number)) {
     ...
     if($(this).attr(settings.maxlength)){
     if(!(longeur<max_length) ){under_max_length=false;}
      }
      if((index>=0)&(Selected.length>0)){

     }else  if(index>=0&under_max_length){
     }else {
         var nom = $(this).val();
         var th=$(this);
        if(nom.length==0){
